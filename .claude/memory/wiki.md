...
- 同一份刚失败的 committed selection 原样重放现为 1 根、163 个 Plan 对象、64/64 资源闭包，完整 Writer/ZIP 构建成功（971443 bytes，18 条均为非阻断 warning）。正例覆盖嵌套 clip、Instance clip、Transform Group，反例继续锁定显式 mask 与 clipsContent 组合必须 fail closed。最终门：Python `1234 passed, 4 skipped, 3 warnings`，Figma plugin `223 passed`，Ruff 与 strict mypy（61 个源码文件）通过；真实 FairyGUI Editor GUI 打开仍待用户执行。
