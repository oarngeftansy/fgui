...
describe("selection upload transaction", () => {
  it("uses the existing idempotent session route order, 64 KiB request bodies, and safe errors", async () => {
    ...
    expect(fetchImpl.mock.calls.map((call: unknown[]) => call[0])).toEqual([
      ...
      "https://fgui.test/v1/figma/selections/uploads/upload/manifest",
      "https://fgui.test/v1/figma/selections/uploads/upload/resources/asset-1",
      "https://fgui.test/v1/figma/selections/uploads/upload/commit",
    ]);
  ...
  it("retries only create and never repeats a failed manifest PUT", async () => {
    ...
    expect(fetchImpl.mock.calls.map((call: unknown[]) => call[0])).toEqual([
      "https://fgui.test/v1/figma/selections/uploads", "https://fgui.test/v1/figma/selections/uploads", "https://fgui.test/v1/figma/selections/uploads/upload/manifest",
    ]);
  });
